		...
		if i == image_path[:len(image_path)-5]:
			...
			for filename in os.listdir('.'):
				if filename.endswith('cat_CORRECTED.ascii'):
					...
					lim_save_path = '/mnt/dwf/archive_NOAO_data/data_outputs/'+year+'/'+month+'/'+field +'/g_band/single/' + filename + '/limiting_mags/'
					plot_save_path = '/mnt/dwf/archive_NOAO_data/data_outputs/'+year+'/'+month+'/'+field +'/g_band/single/' + filename + '/limiting_mags/check_plots/'
					print(lim_save_path)
					...
					plt.ylim(-0.1, 0.4)
					plt.savefig(plot_save_path + filename + 'magerr_vs_mag_plot.png', overwrite = True)
					#plt.show()
